class FifthLevelScene extends eui.Component implements  eui.UIComponent {
	...
	private async onTrashDrop(e: lzlib.LzDragEvent):Promise<void>
	{
		...
		if ((e.data as number) == 7) {
			...
			Main.instance.gotoScene(new SixthLevelScene());
		}
		else{
	...
	{   
		...
		this.returnGroup.addEventListener(egret.TouchEvent.TOUCH_TAP, ()=>Main.instance.gotoScene(new FifthLevelScene()), this);
	...
	{   
		...
		this.no.addEventListener(egret.TouchEvent.TOUCH_TAP, ()=>Main.instance.gotoScene(new FifthLevelScene()), this);
	}
