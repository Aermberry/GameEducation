class EightLevelScene extends eui.Component implements  eui.UIComponent {
	...
	private async onTrashDrop(e: lzlib.LzDragEvent):Promise<void>
	{
		...
		if ((e.data as number) == 2) {
			...
			Main.instance.gotoScene(new NinthLevelScene());
		}
		else{
	...
	{   
		...
		this.returnGroup.addEventListener(egret.TouchEvent.TOUCH_TAP, ()=>Main.instance.gotoScene(new EightLevelScene()), this);
	...
	{   
		...
		this.no.addEventListener(egret.TouchEvent.TOUCH_TAP, ()=>Main.instance.gotoScene(new EightLevelScene()), this);
	}
