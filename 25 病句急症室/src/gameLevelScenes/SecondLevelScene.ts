class SecondLevelScene extends eui.Component implements  eui.UIComponent {
	...
	private async onTrashDrop(e: lzlib.LzDragEvent):Promise<void>
	{
		...
		if ((e.data as number) == 5) {
			...
			Main.instance.gotoScene(new ThiridLevelScene());
		}
		else{
	...
	{   
		...
		this.returnGroup.addEventListener(egret.TouchEvent.TOUCH_TAP, ()=>Main.instance.gotoScene(new SecondLevelScene()), this);
	...
	{   
		...
		this.no.addEventListener(egret.TouchEvent.TOUCH_TAP, ()=>Main.instance.gotoScene(new SecondLevelScene()), this);
	}
