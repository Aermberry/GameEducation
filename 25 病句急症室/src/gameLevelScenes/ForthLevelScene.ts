class ForthLevelScene extends eui.Component implements  eui.UIComponent {
	...
	private async onTrashDrop(e: lzlib.LzDragEvent):Promise<void>
	{
		...
		if ((e.data as number) == 2) {
			...
			Main.instance.gotoScene(new FifthLevelScene());
		}
		else{
	...
	{   
		...
		this.returnGroup.addEventListener(egret.TouchEvent.TOUCH_TAP, ()=>Main.instance.gotoScene(new ForthLevelScene()), this);
	...
	{   
		...
		this.no.addEventListener(egret.TouchEvent.TOUCH_TAP, ()=>Main.instance.gotoScene(new ForthLevelScene()), this);
	}
